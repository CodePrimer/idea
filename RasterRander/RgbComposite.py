    ...
    def Compose(inputPath, bandComb, noDataValue=None, stretchType='LinearPercent', stretchParam=[2], isAlpha=False,
        ...
        if returnMode.upper() == 'GEOTIFF':
            ...
            if isAlpha:
                ...
                imgObj = Image.fromarray(resultArray[:, :, 0:3])
            if resize:
                imgObj = imgObj.resize((int(imgObj.width*resize), int(imgObj.height*resize)), Image.NEAREST)
...
if __name__ == '__main__':
    inFile = r'C:\Users\Administrator\Desktop\BPData\H8_AHI_L2_202011041300_1000_00_00_taihu_chla_empir.tif'
    combine = [1, 4, 4]        # 波段组合
    noData = 65535         # 无效值
    outFile = r'H:\H8_AHI_L2_202011041300_1000_00_00_trueColor.tif'   # 输出路径
    # resizeScale = 0.1       # 行列缩放系数
    abc = RgbComposite.Compose(inFile, combine, noDataValue=noData, returnMode='GEOTIFF', outputPath=outFile,
                               isAlpha=True, stretchParam=[0])
    print('Finish')
